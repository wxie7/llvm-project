...
bool DivergenceLoweringHelper::lowerTemporalDivergenceI1() {
  ...
  SmallDenseMap<std::pair<Register, const MachineCycle *>, Register>
      InstCycleCache;
  for (auto [Reg, UseInst, Cycle] : MUI->getTemporalDivergenceList()) {
    if (MRI->getType(Reg) != LLT::scalar(1))
      continue;

    if (InstCycleCache.contains({Reg, Cycle}))
      continue;
    InstCycleCache[{Reg, Cycle}] = {};
  }

  for (auto InstCycleMapEntry : InstCycleCache) {
    auto InstCycle = InstCycleMapEntry.getFirst();
    Register Reg = InstCycle.first;
  ...
  for (auto [Reg, UseInst, Cycle] : MUI->getTemporalDivergenceList()) {
    ...
    replaceUsesOfRegInInstWith(Reg, UseInst, InstCycleCache[{Reg, Cycle}]);
  }
